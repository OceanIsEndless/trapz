...
(function (BaseGame) {
    var PlayerCtrl = (function (_super) {
        ...
        PlayerCtrl.prototype.doPost = function () {
            ...
            window.game.socket.emit('move', /*JSON.stringify(*/ {
...
(function (BaseGame) {
    var PlayState = (function (_super) {
        ...
        PlayState.prototype.create = function () {
            ...
            this.upgrGr = new BaseGame.UpgradeGroup(this.game);
            this.game.add.existing(this.upgrGr);
           ...
           */
            ...
            window.game.socket = window.io('https://ca1.trapz.io', { transports: ['websocket'] }); ///ca1 - final //ws - dev
            this.startTimePing = new Date();
            window.game.socket.on('connect', function () {
                Constants.MAIN_STATE.endTimePing = new Date();
            });
            Constants.PLAYER_NAME = window.inputName;
            window.game.socket.emit('login', Constants.PLAYER_NAME);
            window.$("#loading").show();
            // (<any>window).game.socket.start()
            //authorize server
        ...
        PlayState.prototype.doLogin = function () {
            window.game.socket.on('login', function (data) {
                ...
                if (Constants.MAIN_STATE.givenLoginID == null || typeof Constants.MAIN_STATE.givenLoginID === 'undefined' || Constants.MAIN_STATE.givenLoginID.length == 0) {
                    ...
                    window.game.socket.on('update', function (msg) {
                        // console.log('update: ' + msg);
                        Constants.MAIN_STATE.parseServerData(msg);
                    });
                    // save cookie
                    console.log('NEW ID set ' + Constants.MAIN_STATE.givenLoginID);
                    ...
                    var time = Constants.MAIN_STATE.endTimePing.getTime() - Constants.MAIN_STATE.startTimePing.getTime();
                    window.game.socket.emit('stats', {
                        id: Constants.MAIN_STATE.givenLoginID, connecttime: time
                    });
                    window.game.socket.on('wallet', function (data) {
                        /*
                        {
                    ...
                    //end wallet
                    window.game.socket.on('death', function (data) {
                ...
                window.game.socket.on('disconnect', function (data) {
                    window.game.socket.emit('reconnect', { auth: Constants.MAIN_STATE.authInfo });
                });
                window.game.socket.on('reconnected', function (data) {
                    console.log('reconnected');
                });
                window.game.socket.on('login_required', function (data) {
        ...
        PlayState.prototype.tryAttack = function () {
            if (this.game.time.now > this.attackAllowedTimer) {
                ...
                window.game.socket.emit('attack', { type: 'sword', auth: Constants.MAIN_STATE.authInfo });
                ...
                for (var i = 0; i < window.game.enemies.length; i++) {
                    ...
                    for (var e = 0; e < data.length; e++) {
                        if (playerUnit.nameID == data[e].user.name && playerUnit.sessionID == data[e].user.session) {
                            ...
                            if (indx != -1) {
                                ...
                                console.log('in copy remove indx ' + indx);
                            }
                        }
                    }
                    //end data loop
                    if (!found) {
                        ...
                        if (!playerUnit.isShowingDeathAnim) {
                            ...
                            console.log("SHOW DEATH ANIM, haven't found in a new array name " + playerUnit.nameID);
                        }
                    }
                }
        ...
        PlayState.prototype.makeSpr = function (withPhysics) {
        ...
        PlayState.prototype.makeEnemyPlayer = function (data, thisPlayer) {
            ...
            /*
             ...
             if (existPlayer != null)
                 return;*/
            //id, startx, starty
            var spr = Constants.MAIN_STATE.makeSpr(thisPlayer);
        ...
        PlayState.prototype.endGame = function () {
            ...
            */
            ...
            if (this.allFps != 0) {
                window.game.socket.emit('stats', {
                    minfps: window.game.time.fpsMin, avgfps: Math.floor(this.allFps / this.countedFrames), maxfps: window.game.time.fpsMax, id: Constants.MAIN_STATE.givenLoginID
                });
            }
            this.allFps = 0;
            this.countedFrames = 0;
    ...
    var UpgradeGroup = (function (_super) {
        ...
        UpgradeGroup.prototype.renewInfoText = function () {
            if (typeof this.textNamesSpr != 'undefined') {
                ...
                this.textNamesSpr.text = '' + Constants.MAIN_STATE.player.nameID;
        ...
        UpgradeGroup.prototype.swordUpgrade = function () {
            if (Constants.MAIN_STATE.player.score >= this.needPriceForSword()) {
                ...
                window.game.socket.emit('upgradeBuy', {
                    auth: Constants.MAIN_STATE.authInfo,
                    type: 'attack'
                });
